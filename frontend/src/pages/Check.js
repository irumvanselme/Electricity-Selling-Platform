    ...
    const checkElectricty = async () => {
        try {
            let { data } = await axios.get(
                `http://localhost:8000/api/meters/by-number/${meter_number}`
            );
    ...
    return (
        <div id="_check_balance_page">
            ...
            {error !== "" && (
                <div
                    title="Error"
                    className="py-10 text-red-500 w-[400px]"
                    id="ErrorMessage"
                >
                    {error}
                </div>
            ...
            {isSuccess && (
                ...
                >
                    Success you have electirity for {days} days
                </div>
            )}
            <div>
                <Input
                    ...
                    title="meter_number_input"
                    data={{ st: meter_number, sts: setMeterNumber }}
                />

                <Button
                    action="checkElectricity"
                    title="Click to Check"
                    onClick={checkElectricty}
                />
            </div>
        </div>
    );
